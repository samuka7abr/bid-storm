...
func TestNewEngineBuildsTheOptimisticEngine(t *testing.T) {
	// No pool is dialled here: selecting an engine must not touch the database,
	// or the boot would fail for the wrong reason when Postgres is slow to come
	// up under the compose.
	engine, err := app.NewEngine(app.StrategyOptimistic, nil, metrics.NewRegistry())
	if err != nil {
...
func TestNewEngineRefusesWhatDoesNotExistYet(t *testing.T) {
	tests := []struct {
		...
		{app.StrategyPessimistic, "etapa 2"},
